  ...
  return (
    <li className="relative flex items-center justify-between space-x-2 pr-4 text-xl">
      <button
        ...
        type="button"
        className="my-3 rounded-md p-0.5 text-red-500 hover:bg-gray-100 dark:hover:bg-gray-500"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-6 w-6">
          <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg>
      </button>
      <div className={`${todo.completed ? 'text-red-500 line-through dark:text-red-400' : ' text-gray-800 dark:text-white'} flex flex-auto flex-col p-1`}>
        <span>{todo.title}</span>
        <span className={`${!todo.completed && ' text-gray-600 dark:text-gray-400'} pl-2 text-base`}>{todo.dueDate || 'N/A'}</span>
      </div>
      <input
