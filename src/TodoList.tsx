...
const TodoList = ({ todoList, setTodoList }: TodoListProps) => {
  ...
  return (
    <>
      ...
      <div className="flex flex-auto flex-col items-center space-y-3 p-4 sm:items-start">
        <select
        ...
        <div className="z-10 flex h-full w-full flex-auto flex-col justify-center px-2">
          <ul className="w-full space-y-1 divide-y divide-gray-300 dark:divide-gray-700">
            {todoList.length === 0 && <li className="p-4 text-center text-2xl text-gray-800 dark:text-white">There are no Todos</li>}
            {todoList.length > 0 &&
